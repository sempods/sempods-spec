...
| `registered` | a service client'"'"'s grants, fixed when it was registered. Not policy — no access control resource carries them — and they take the place of both the ceiling and the context decision, so a request whose subject *is* its client is answered from them |
