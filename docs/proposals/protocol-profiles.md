...
## JSON-LD and RDF conversion

JSON-LD 1.1 syntax §10.1 is an informative overview of both conversion directions. For the
selected shapes, incorporate the RDF/term conversion rules in the dated
[Processing Algorithms and API §§8.1.2/8.2.2 and 8.4.2/8.5.2](https://www.w3.org/TR/2020/REC-json-ld11-api-20200716/#rdf-serialization-deserialization-algorithms).
These supply the input and output mapping, including IRI objects, datatype IRIs and language tags.
Require equivalent results, not a particular algorithm implementation or the general processor API.

The sempods shape, validation and routing rules remain explicit constraints: CRUD-012 chooses the
write context, CRUD-023/024 choose the output shape, CRUD-025 forbids prefix resolution, CRUD-032
rejects other subjects, and CRUD-035–038 apply Merge Patch before RDF conversion. Slot values use
the same term mapping under CRUD-041. Conversion does not authorize silently dropping input that
these rules require rejecting, fetching contexts or accepting additional JSON-LD shapes. Raw media
uploads remain bytes and write no RDF under MEDIA-005; an application's separate RDF write uses CRUD.
...
| P07 | Public registration with `redirect_uris`, method `none`, grant/response types omitted | `201` with a `dyn:` client identifier under AUTH-008; defaults are code flow. Return actual registered metadata; no service credentials or promise of refresh issuance. |
...
| P12 | `did:web:app.example:notes` with `https://app.example/notes/cb` versus `/notes-other/cb` | First succeeds with S256 and consent; second is refused without redirecting to the rejected URI. Same host alone is insufficient. |
...
| P14 | OIDC pod client uses its permitted origin for code callback and `none` client authentication; repeat with a changed callback at exchange, then independently with mismatching discovery issuer, invalid ID Token issuer/audience/nonce/signature/expiry or replayed callback state | Valid code/PKCE flow succeeds without preregistration or a shared secret. Changed exchange callback gives `invalid_grant` under Core §3.1.3.2; the origin exception does not extend to redemption. Discovery mismatch fails metadata validation; each assertion/state failure rejects the login without resuming parked authorization under OIDC-006/015. |
...
| P34 | Authorized resource PUT contains values `{"@id":"urn:example:object"}`, `{"@value":"007","@type":"urn:example:code"}` and `{"@value":"Hallo","@language":"de"}` under absolute predicate IRIs; then GET it. Repeat through slot writes and a valid Merge Patch. | Writes succeed under the existing endpoint status rules and produce the corresponding RDF IRI, typed literal and language-tagged literal. The read retains those terms in the canonical value-object shape, including lexical value `007`, datatype `urn:example:code` and language `de`; array order is immaterial. Existing scope rules still apply. |
