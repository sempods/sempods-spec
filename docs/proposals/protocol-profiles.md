...
and the adopted MCP-009 challenge. Preserve the upstream obligation levels for optional sessions
and error bodies rather than hardening a suggested status into an unconditional requirement.
...
| P17 | MCP POST contains truncated JSON with unreadable ID | Parse error `-32700` without `id` under the selected MCP profile; neither `id:null` nor a result is a valid proposed MCP error envelope. |
...
| P19 | Invalid Origin at M | HTTP `403`; the optional transport error body can omit ID even if the unprocessed body contains one. |
| P20 | GET M on a server without SSE; DELETE of a session whose server declines client termination | `405` with Allow for the unsupported operation; neither SSE nor client-requested session deletion becomes mandatory. |
