...
<a id="SPS-AUTH-025"></a>
**`SPS-AUTH-025`** — The implementation MUST handle authorization success and error responses per
[RFC 6749 §§4.1.2 and 4.1.2.1](https://www.rfc-editor.org/rfc/rfc6749.html#section-4.1.2),
including validation of the client's `redirect_uri` and, in responses sent to that client,
echoing `state` unchanged only when supplied in the authorization request.
...
without it, neither adds one. For a missing or invalid client identifier, or a missing, invalid or
mismatching redirect URI, the RFC prohibits automatically redirecting to the invalid client URI.
How the server informs the resource owner, including a server-owned error page, its status and
representation, is implementation-defined. Optional `state` does not remove the client's CSRF
...
one, MUST each write before they check, so that whichever lands second sees the first: the exchange
...
<a id="SPS-AUTH-061"></a>
**`SPS-AUTH-061`** — An implementation that revokes a person's refresh-token families MUST reach
every equivalent identity URI it holds for them, not only the URI the request carries.

Revoking a grant is a write, so [`SPS-AUTH-052`](#SPS-AUTH-052) already covers it. A refresh-token
family is not a grant, and without this one issued under a person's other URI survives their
withdrawal — the survivor being the one nobody was looking at. The read path is untouched: this is
what a revocation covers, not how a request is answered.
...
The endpoint-only MCP entry establishes that URL through
[SPS-MCP-033](../modules/mcp.md#SPS-MCP-033). The pod is both the protected resource and the token
issuer (SPS-AUTH-028).
...
[RFC 8414 §§2–4 and 6](https://www.rfc-editor.org/rfc/rfc8414.html#section-2) for authorization
server metadata. SPS-AUTH-067/068 define the pod-local address and resource-validation deviations;
SPS-AUTH-069 identifies the expected authorization-server issuer.
Optional metadata fields retain their standard meanings; they add no required OAuth flow.
...
<a id="SPS-AUTH-065"></a>
**`SPS-AUTH-065`** — The pod's resource metadata MUST set `resource` to its canonical base URL,
set `authorization_servers` to an array containing only that URL, and include `header` in
`bearer_methods_supported`.

Fetching the example's metadata URL without credentials gives `200 application/json`:
...
<a id="SPS-AUTH-066"></a>
**`SPS-AUTH-066`** — An implementation MUST serve Authorization Server Metadata at
`GET {pod}/.well-known/oauth-authorization-server`, without authentication. The document MUST
set `issuer` to the canonical pod base URL and advertise the chapter's authorization, token,
registration and signing-key endpoints in `authorization_endpoint`, `token_endpoint`,
`registration_endpoint` and `jwks_uri`. It MUST explicitly list the supported response types,
grant types, token-endpoint authentication methods and PKCE methods in their RFC 8414 fields.

For Alice, the metadata URL is
`https://example.org/alice/.well-known/oauth-authorization-server`. Its `issuer` is
`https://example.org/alice`; its `token_endpoint` is `https://example.org/alice/_system/auth/token`.
The issuer identifies the pod, not its auth route. The metadata describes the flows already
required by this chapter; SPS-AUTH-048 still limits service-client advertisement.
...
<a id="SPS-AUTH-067"></a>
**`SPS-AUTH-067`** — For pod-local discovery, servers and clients MUST append
`/.well-known/oauth-protected-resource` or `/.well-known/oauth-authorization-server` to the
canonical pod base URL. This replaces the host-rooted construction in RFC 9728 §§3/3.1 and
...
<a id="SPS-AUTH-068"></a>
**`SPS-AUTH-068`** — A client using this profile MUST validate resource discovery against the
canonical pod base URL it intended to access, established independently of returned metadata.
It MUST reject a challenge whose `resource_metadata` differs from that pod's append-form URL,
or resource metadata whose `resource` or sole `authorization_servers` entry differs from that
pod URL. In RFC 9728 §3.3 validation, the expected resource MUST be this pod URL for both data
and MCP endpoints, replacing endpoint-URL comparison. A client MUST NOT use rejected metadata
to start authorization or send credentials; exact string comparison and TLS checks still apply.

<a id="SPS-AUTH-069"></a>
**`SPS-AUTH-069`** — A client using this profile MUST validate authorization-server metadata with
the intended canonical pod base URL as the expected issuer under RFC 8414 §3.3, and MUST NOT use
metadata with a different `issuer` to start authorization or send credentials.

The known pod identity supplies the boundary: Alice's challenge cannot redirect a client to Bob's
issuer. Matching an origin or a parent path is insufficient. The MCP endpoint-only entry obtains
that identity from the specified endpoint layout before applying these checks; it does not adopt
an identity returned by a metadata document. A deployment-specific metadata alias supplies no
exception to these validation rules.

### Cases to check

These are contract examples, not results from a running implementation.

| Case | Expected result |
|---|---|
| Protected write without a token, on a pod without MCP | `401` with the pod metadata hint; the two public metadata GETs identify its authorization endpoints. |
| Public read without a token | Normal public response; discovery does not force an authentication challenge. |
| Public read with a rejected bearer | `401` with the same pod metadata hint; no anonymous fallback. |
| Metadata GET without credentials | `200 application/json`; no context IRIs, including public ones. Unknown fields remain tolerable. |
...
| Client knows only the canonical sempods MCP endpoint | Establish the pod base under SPS-MCP-033, then use the same pod-local metadata and exact checks. A generic MCP client needs explicit support for this sempods profile. |
